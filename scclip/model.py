...
class CLIPLoss(nn.Module):
    def __init__(self, temp=2.6592, requires_grad=False):
        super().__init__()
        self.logit_scale = nn.Parameter(torch.ones([]) * temp, requires_grad=requires_grad)
...
class Tokenizer(nn.Module):
    ...
    def __init__(self, config, feature_size, use_mask_token: bool = False) -> None:
        ...
        self.patch_embeddings = PatchEmbeddings(feature_size, config.num_patches, config.hidden_size)
...
class TransformerModel(nn.Module):
    ...
    def __init__(self, config, feature_size):
        ...
        encoder_layers = TransformerEncoderLayer(config.hidden_size, config.num_heads, config.ffn_dim, config.dropout)
