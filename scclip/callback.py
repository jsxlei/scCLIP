...
class Monitor(Callback):
    def __init__(self, dm, metric='euclidean'): 
        ...
        self.example_array['cell_type'] = self.dataset.le.inverse_transform(self.example_array['cell_type'])
        self.sort_by_labels()
    ...
    def save_checkpoint(self, trainer):
        trainer.save_checkpoint(os.path.join(trainer.logger.log_dir, 'checkpoints', 'epoch={}-step={}.ckpt'.format(trainer.current_epoch, trainer.global_step)))
        

    @torch.no_grad()
    def get_output(self, pl_module):
        atac_embeds, rna_embeds = pl_module(self.example_array['atac'], self.example_array['rna'])
        ...
        concat_labels = np.concatenate([self.example_array['cell_type'], self.example_array['cell_type']])
        domain_labels = np.array(['atac']*len(atac_embeds)+ ['rna']*len(rna_embeds))
        obs = pd.DataFrame([concat_labels, domain_labels], index=['cell_type', 'modality']).T
        paired = AnnData(concat_embeds, obs=obs)
        
        return paired

    @torch.no_grad()
    def show_umap(self, pl_module, metric=None, show=True, norm=False, score=False, **kwargs):
    ...
    def show_corr(self, pl_module, show=True, metric=None, norm=False):
        ...
        X = pairwise_distances(paired.X, metric=metric) #[:len(atac_embeds), len(atac_embeds):]
        fig = plot_heatmap(X, obs_names=paired.obs['cell_type'], var_names=paired.obs['cell_type'], sort=False, cmap='RdBu')
    ...
    def show_predict(self, pl_module, show=True): # takes 1s
        ...
        for name, array in self.example_array.items():
            pred = pl_module(array[0].to(pl_module.device)).to('cpu').detach().numpy().squeeze()

            fig = plot_heatmap(pred[:, self.index[name]['gene']], obs_names=self.obs_names[name], vmax=5.5)
            if show:
            ...
            score = [scipy.stats.pearsonr(pred[:, i], array[1][:, i])[0] for i in range(array[1].shape[1])]
            g = sns.displot(score)
            ...
            if show:
                ...
                experiment.add_figure('pearson_coef/{}'.format(name), plt.gcf(), current_epoch)
    ...
    def show_attention_maps(self, pl_module, show=True): # takes 41.6s
        ...
        if show:
            ...
            pl_module.logger.experiment.add_figure("attention/"+name, fig, pl_module.current_epoch)
