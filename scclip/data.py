...
class BaseDataset(Dataset):
    def __init__(
        ...
        print('RNA shape', self.mdata.mod['rna'].shape, 'atac shape', self.mdata.mod['atac'].shape, flush=True)
    ...
    def read(self):
        if self.modality == 'multiome':
            assert os.path.isfile(os.path.join(self.data_dir)), f'MultiOme file {self.data_dir} not found'
    ...
    def _split(self, split):
        if isinstance(split, float):
            ...
            if split.endswith('_r'):
                ...
                reverse = False
            assert split in self.mdata.obs['cell_type'].unique(), f'Cell type {split} not found'
            if reverse:
    ...
    def _preprocess_rna(self):
        ...
        if isinstance(self.n_top_genes, int):
            if self.n_top_genes > 0:
                sc.pp.highly_variable_genes(rna, n_top_genes=self.n_top_genes, inplace=False, subset=True) # batch_key='batch',
    ...
    def concat_mudata(self):
        ...
        if self.modality == 'multiome':
            keys = []
            atac = concat([mdata.mod['atac'] for mdata in self.mdata], label='dataset', keys=self.data_dir, index_unique='#')
            rna = concat([mdata.mod['rna'] for mdata in self.mdata], label='dataset', keys=self.data_dir, index_unique='#')
...
class MTDataset(Dataset):
    def __init__(
        ...
        **kwargs
        ...
        print(_keys)
        self.datasets = {}
        self.datasets['atac'] = ATACDataset(_keys['atac'][0], **kwargs) if len(_keys['atac']) == 1 else ConcatDataset([ATACDataset(name, **kwargs) for name in _keys['atac']])
        self.datasets['rna'] = RNADataset(_keys['rna'][0], **kwargs) if len(_keys['rna']) == 1 else ConcatDataset([ATACDataset(name, **kwargs) for name in _keys['rna']])
        self.datasets['multiome'] = MultiOmeDataset(_keys['multiome'][0], **kwargs) if len(_keys['multiome']) == 1 else ConcatDataset([MultiOmeDataset(name, **kwargs) for name in _keys['multiome']])
        print(len(self.datasets['atac']), len(self.datasets['rna']), len(self.datasets['multiome']))
    ...
    def __len__(self):
        return min([len(self.datasets['atac']), len(self.datasets['rna']), len(self.datasets['multiome'])])
...
_atac_datasets = {
    ...
    'atlas': atac_dir / 'Cell_zhang_GSE184462/atac_atlas.h5ad', # Cell paper and Science paper
...
class MTDataModule(BaseDataModule):
    def __init__(self, _config, dist=False):
        ...
        self.dm_dicts = {key: _datamodules[key.split('_')[0]](_datasets[key.split('_')[0]][key.split('_', 1)[1]]) for key in datamodule_keys}
