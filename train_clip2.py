...
from scclip.model import scCLIPModel
...
if __name__ == '__main__':
    ...
    if args.checkpoint is not None:
        model = scCLIPModel.load_from_checkpoint(args.checkpoint) 
        ...
        args.n_peaks = dm.dataset.mdata.mod['atac'].shape[1]
        args.n_genes = dm.dataset.mdata.mod['rna'].shape[1]
        model = scCLIPModel(args) 

        # out_dir
        args.default_root_dir = f'results/{args.data_dir}/{args.temp}'
    ...
    # results
    args.default_root_dir = str(args.default_root_dir)
    # get_results(model, dataset.dataloader(), trainer, args)
